...
protected:
    void SetUp() override {
        test_output_path_ = "/tmp/test_export.txt";
    }
...
TEST_F(TextExporterTest, GetFormat) {
    const TextExporter exporter;
    EXPECT_EQ(exporter.get_format(), ExportFormat::TEXT);
}

TEST_F(TextExporterTest, GetDefaultExtension) {
    const TextExporter exporter;
    EXPECT_EQ(exporter.get_default_extension(), ".txt");
}

TEST_F(TextExporterTest, ExportContainsMetrics) {
    TextExporter exporter;
    ...
    std::string txt_content = read_file(test_output_path_);

    EXPECT_TRUE(txt_content.find("test1.cpp") != std::string::npos);
}

TEST_F(TextExporterTest, WithColors) {
    TextExporter::Options options;
    options.use_colors = true;
    TextExporter exporter(options);
...
TEST_F(TextExporterTest, WithoutColors) {
    TextExporter::Options options;
...
TEST_F(TextExporterTest, CompactMode) {
    TextExporter::Options options;
...
TEST_F(TextExporterTest, ExpandedMode) {
    TextExporter::Options options;
...
TEST_F(TextExporterTest, CustomPathLength) {
    TextExporter::Options options;
    options.max_path_length = 40;
    TextExporter exporter(options);
