...
protected:
    void SetUp() override {
        test_output_path_ = "/tmp/test_export.md";
    }
...
TEST_F(MarkdownExporterTest, BasicExport) {
...
TEST_F(MarkdownExporterTest, GetFormat) {
    const MarkdownExporter exporter;
    EXPECT_EQ(exporter.get_format(), ExportFormat::MARKDOWN);
}

TEST_F(MarkdownExporterTest, GetDefaultExtension) {
    const MarkdownExporter exporter;
    EXPECT_EQ(exporter.get_default_extension(), ".md");
}

TEST_F(MarkdownExporterTest, ContainsMarkdownSyntax) {
    MarkdownExporter exporter;
    ...
    std::string md_content = read_file(test_output_path_);

    EXPECT_TRUE(md_content.find('#') != std::string::npos);
}

TEST_F(MarkdownExporterTest, WithTableOfContents) {
    MarkdownExporter::Options options;
    options.include_toc = true;
    MarkdownExporter exporter(options);
...
TEST_F(MarkdownExporterTest, WithoutTableOfContents) {
    MarkdownExporter::Options options;
...
TEST_F(MarkdownExporterTest, ExportContainsMetrics) {
    MarkdownExporter exporter;
    ...
    EXPECT_FALSE(md_content.empty());
    EXPECT_GT(md_content.length(), 0);
}

TEST_F(MarkdownExporterTest, ExportContainsTable) {
    MarkdownExporter exporter;
...
TEST_F(MarkdownExporterTest, WithCodeBlocks) {
    MarkdownExporter::Options options;
    options.include_code_blocks = true;
    MarkdownExporter exporter(options);
