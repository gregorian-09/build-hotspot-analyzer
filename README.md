...
For nontrivial repository validation, `tests/clone_repos.sh`, `tests/build_repos.sh`, `tests/run_repo_apply_benchmark.py`, and targeted `tests/run_*_subproject.py` harnesses exercise real and synthetic projects. The benchmark runner accepts one or more `--project` names from `tests/cli/repos`, creates a detached Git worktree for each, captures compiler traces, applies one eligible analyzed suggestion ID through `bha project apply`, and compares median **untraced** `cmake --build --clean-first` wall time before and after the edit. CMake configuration time is excluded; each state is reconfigured outside the timed build. It keeps worktrees, traces, logs, JSON, and a Markdown summary under `tests/cli/benchmarks/` for review; it never resets the original clone. Run `python3 tests/run_repo_apply_benchmark.py --help` for workload and test-gate controls. Legacy project profiles often disable tests; use `--cmake-arg` to override their CMake options and `--require-tests` to make missing CTest coverage a failure. Use `--require-applied` to fail if no direct, auto-applicable suggestion was applied and validated. `build_only` means the project configured no CTest tests, so a compile pass is the only validation. Timing comparisons should retain raw traces, project revision, compiler/version, build command, cache state, and machine context; a single successful fixture does not establish general optimization quality.

The manually dispatched [repository suggestion benchmark](.github/workflows/repo-apply-benchmark.yml) runs the selected pinned LevelDB or yaml-cpp revision independently on Linux Clang, Windows VS 2022 MSVC, and macOS AppleClang; `all` runs both projects on every OS. Each leg requires a real trace, an eligible direct edit, post-edit build validation, and CTest coverage. A missing eligible suggestion is a failed edit check, not a successful optimization. Jobs are capped at two concurrent project legs and upload per-project result JSON, timing summaries, and logs. The same workflow also calls the [VS Code extension-host matrix](.github/workflows/vscode-extension.yml), which checks command behavior with a fake LSP server on all three OSes. That host check is separate from the real CLI project-apply benchmark: it does **not** yet prove end-to-end VS Code application against those upstream projects. One clean build per state is a pipeline smoke measurement, not statistically reliable savings evidence.
