...
private:
    BuildMetrics extract_build_metrics(const BuildTrace& trace);

    [[nodiscard]] Priority calculate_priority(double improvement_percentage) const;

    Suggestion convert_suggestion(const bha::Suggestion& bha_sug);
    DetailedSuggestion convert_to_detailed(const bha::Suggestion& bha_sug);

    std::string generate_analysis_id();
    std::string generate_backup_id();

    std::string create_backup(const std::vector<fs::path>& files);
    bool validate_files_exist(const std::vector<fs::path>& files);
    bool apply_file_changes(const bha::Suggestion& suggestion, std::vector<fs::path>& changed_files);
