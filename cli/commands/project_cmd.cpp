...
namespace bha::cli
{
    ...
    namespace {
        ...
        JsonValue make_apply_all_result_json(const lsp::ApplyAllResult& result) {
            JsonValue errors_json = JsonValue::array();
            for (const auto& diagnostic : result.errors) {
                errors_json.push_back(diagnostic_to_json(diagnostic));
            }
    ...
    private:
        int execute_record(const ParsedArgs& args, const fs::path& project_root) {
            ...
            if (const auto override_name = args.get("build-system"); override_name.has_value()) {
                adapter = registry.get(*override_name);
                if (adapter == nullptr) {
                    print_error("Unknown build system: " + *override_name);
                    return 1;
                }
            } else {
                adapter = registry.detect(project_root);
                if (adapter == nullptr) {
        ...
        int execute_analyze(
            ...
            auto analysis = manager.analyze_project(
                ...
                trace_dir,
                rebuild,
                [this](const std::string& message, const int) {
                    if (is_verbose()) {
        ...
        int execute_apply(
            ...
            if (args.get_flag("all")) {
                ...
                if (is_json()) {
                    std::cout << make_apply_all_result_json(result).dump(2) << "\n";
                } else {
