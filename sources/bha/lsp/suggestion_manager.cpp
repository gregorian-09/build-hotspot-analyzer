...
AnalysisResult SuggestionManager::analyze_project(
    ...
    if (rebuild) {
        ...
        if (!build_result.is_ok() || !build_result.value().success) {
            throw std::runtime_error("Build failed");
        }
    }

    auto compile_commands_result = adapter->get_compile_commands(project_root, options);
    if (!compile_commands_result.is_ok()) {
...
ApplySuggestionResult SuggestionManager::apply_all_suggestions(
    ...
    for (const auto& id : suggestion_ids) {
        auto result = apply_suggestion(id);
        if (!result.success) {
...
bool SuggestionManager::revert_changes(const std::string& backup_id) {
    ...
    for (const auto& file_backup : backup.files) {
        try {
            std::ofstream out(file_backup.path, std::ios::binary);
            if (!out) {
                return false;
            }
            out << file_backup.content;
        } catch (...) {
            return false;
        }
    }
...
bool SuggestionManager::apply_file_changes(const bha::Suggestion& suggestion, std::vector<fs::path>& changed_files) {
    auto apply_file_target = [&](const bha::FileTarget& target) -> bool {
        try {
            if (target.action == bha::FileAction::Create) {
            ...
            else if (target.action == bha::FileAction::Modify) {
                if (!fs::exists(target.path)) {
                    return false;
                }
                if (!suggestion.after_code.code.empty()) {
            ...
            else if (target.action == bha::FileAction::AddInclude) {
                if (!fs::exists(target.path)) {
                    return false;
                }
                ...
                if (target.note && !target.note->empty()) {
                    size_t first_include = content.find("#include");
    ...
    if (!apply_file_target(suggestion.target_file)) {
        return false;
    }
    for (const auto& secondary : suggestion.secondary_files) {
        if (!apply_file_target(secondary)) {
            return false;
        }
    }
...
std::vector<Suggestion> SuggestionManager::get_all_suggestions() const {
    ...
    for (const auto& [id, detailed] : suggestions_) {
        result.push_back(static_cast<const Suggestion&>(detailed));
    }
    return result;
}
...
BuildMetrics SuggestionManager::extract_build_metrics(const BuildTrace& trace) {
    ...
    auto total_ms = std::chrono::duration_cast<std::chrono::milliseconds>(trace.total_time).count();
    ...
    for (const auto& unit : trace.units) {
        auto unit_ms = std::chrono::duration_cast<std::chrono::milliseconds>(unit.metrics.total_time).count();
    ...
    for (size_t i = 0; i < std::min(file_times.size(), size_t(10)); ++i) {
...
Priority SuggestionManager::calculate_priority(double improvement_percentage) const {
...
Suggestion SuggestionManager::convert_suggestion(const bha::Suggestion& bha_sug) {
    ...
    // Map SuggestionType from bha to lsp
    switch (bha_sug.type) {
        ...
        case bha::SuggestionType::MoveToCpp:
            ...
            break;
        case bha::SuggestionType::ModuleMigration:
            lsp_sug.type = SuggestionType::ModuleMigration;
            break;
        case bha::SuggestionType::InlineReduction:
            lsp_sug.type = SuggestionType::InlineReduction;
            break;
        case bha::SuggestionType::CompilationFirewall:
            lsp_sug.type = SuggestionType::CompilationFirewall;
            break;
        case bha::SuggestionType::DependencyInversion:
            lsp_sug.type = SuggestionType::DependencyInversion;
            break;
        case bha::SuggestionType::SymbolVisibility:
            lsp_sug.type = SuggestionType::SymbolVisibility;
            break;
    }
    ...
    // Convert impact
    auto savings_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    lsp_sug.estimated_impact.files_affected = static_cast<int>(bha_sug.impact.total_files_affected);

    // Map complexity based on implementation steps count
    if (bha_sug.implementation_steps.size() <= 2) {
        lsp_sug.estimated_impact.complexity = Complexity::Simple;
    } else if (bha_sug.implementation_steps.size() <= 5) {
        lsp_sug.estimated_impact.complexity = Complexity::Moderate;
    } else {
        lsp_sug.estimated_impact.complexity = Complexity::Complex;
    }
...
DetailedSuggestion SuggestionManager::convert_to_detailed(const bha::Suggestion& bha_sug) {
    ...
    for (const auto& file : bha_sug.secondary_files) {
        if (file.action == bha::FileAction::Modify ||
            file.action == bha::FileAction::AddInclude) {
            detailed.files_to_modify.push_back(file.path.string());
        }
    }

    // Store implementation steps as dependencies
