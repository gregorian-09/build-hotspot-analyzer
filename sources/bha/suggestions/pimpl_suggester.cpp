    ...
    namespace {
        ...
        std::vector<std::string> split_top_level_initializer_entries(const std::string& initializer) {
            ...
            for (const char ch : initializer) {
                ...
                if (ch == ',' &&
                    ...
                    bracket_depth == 0 &&
                    angle_depth == 0) {
                    auto entry = utils::trim_copy(current);
                    if (!entry.empty()) {
            ...
            auto tail = utils::trim_copy(current);
            if (!tail.empty()) {
        ...
        std::optional<OutOfLineFunctionBlock> find_out_of_line_function_block(
            ...
            for (std::size_t sig_line = 0; sig_line < lines.size(); ++sig_line) {
                ...
                while (start_line > 0) {
                    const std::string previous = utils::trim_copy(lines[start_line - 1]);
                ...
                if (colon_pos.has_value()) {
                    block.signature = utils::trim_copy(header.substr(0, *colon_pos));
                    block.initializer = utils::trim_copy(header.substr(*colon_pos + 1));
                } else {
                    block.signature = utils::trim_copy(header);
                }
                    ...
                    for (std::size_t col = col_begin; col < line.size(); ++col) {
                        ...
                        if (ch == '}') {
                            ...
                            if (brace_depth == 0) {
                                ...
                                block.body = utils::trim_copy(body);
                                return block;
                            }
                            body.push_back(ch);
                            continue;
            ...
            if (!ctor_block.initializer.empty()) {
                ...
                for (const auto& entry : entries) {
                    ...
                    assignments.push_back(
                        "    pimpl_->" + field_name + " = " +
                        utils::trim_copy(rewrite_copy_body_for_pimpl(expression, private_fields)) + ";"
                    );
                }
            }

            const std::string rewritten_body =
                utils::trim_copy(rewrite_copy_body_for_pimpl(ctor_block.body, private_fields));
        ...
        std::optional<std::string> build_rewritten_explicit_copy_assign(
            ...
            std::string rewritten_body = utils::trim_copy(rewrite_copy_body_for_pimpl(assign_block.body, private_fields));
