...
namespace bha::suggestions
{
    namespace {
        ...
        std::optional<MesonTargetSpan> find_first_meson_target(const std::string& content) {
            if (auto span = utils::find_first_meson_target_span(content)) {
                return MesonTargetSpan{span->start_line, span->end_line, span->single_line};
            }
            return std::nullopt;
        }
    ...
    Result<SuggestionResult, Error> TemplateSuggester::suggest(
        ...
        // Timing evidence without AST validation is never sufficient to generate
        // an extern/explicit-instantiation pair.
        if (context.trace.template_evidence == TemplateEvidence::AggregateTiming ||
            (context.trace.template_evidence != TemplateEvidence::None &&
